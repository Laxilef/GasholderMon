...
protected:
  void setup() {
    ...
    new (&wmHostname) WiFiManagerParameter("hostname", "Hostname", settings.hostname, 80);
    wm.addParameter(&wmHostname);

    new (&wmMqttServer) WiFiManagerParameter("mqtt_server", "MQTT server", settings.mqtt.server, 80);
    wm.addParameter(&wmMqttServer);

    char mqttPort[6];
    sprintf(mqttPort, "%d", settings.mqtt.port);
    new (&wmMqttPort) WiFiManagerParameter("mqtt_port", "MQTT port", mqttPort, 6);
    wm.addParameter(&wmMqttPort);

    new (&wmMqttUser) WiFiManagerParameter("mqtt_user", "MQTT username", settings.mqtt.user, 32);
    wm.addParameter(&wmMqttUser);

    new (&wmMqttPassword) WiFiManagerParameter("mqtt_password", "MQTT password", settings.mqtt.password, 32);
    wm.addParameter(&wmMqttPassword);

    new (&wmMqttPrefix) WiFiManagerParameter("mqtt_prefix", "MQTT prefix", settings.mqtt.prefix, 32);
